...
class UsuarioModel {
    ...
    public function buscarPorId($id_user){
        $query = "SELECT * FROM $this->tabela WHERE id_user = :id_user";
        $stmt = $this->conn->prepare($query);
    ...
    public function criar($usuario) {
        $query = "INSERT INTO $this->tabela (nome, email, data_nasc, cpf, telefone) VALUES (:nome, :email, :data_nasc, :cpf, :telefone)";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(":nome", $usuario["nome"]);
        $stmt->bindParam(":email", $usuario["email"]);
        $stmt->bindParam(":data_nasc", $usuario["data_nasc"]);
    ...
    public function editar($usuario){
        $query = "UPDATE $this->tabela SET nome=:nome, email=:email, data_nasc= :data_nasc, cpf=:cpf, telefone=:telefone WHERE id_user = :id_user";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(":nome", $usuario["nome"]);
        $stmt->bindParam(":email", $usuario["email"]);
        $stmt->bindParam(":data_nasc", $usuario["data_nasc"]);
        $stmt->bindParam(":cpf", $usuario["cpf"]);
        $stmt->bindParam(":telefone", $usuario["telefone"]); 
        $stmt->bindParam(":id_user", $usuario["id_user"]); 
        return $stmt->execute(); 
    ...
    public function excluir($id_user){
        $query=" DELETE FROM $this->tabela WHERE id_user = :id_user";
        $stmt = $this->conn->prepare($query);
